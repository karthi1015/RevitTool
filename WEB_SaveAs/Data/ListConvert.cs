...
namespace WEB_SaveAs
{
    ...
    public class TextConverter : IMultiValueConverter
    {
        ...
        {
            level_block level_Block = (level_block)values[0];
            bool option_normal = (bool)values[1];
            string text1 = "";
            if (level_Block != null)
            {
                text1 = level_Block.number + "_" + level_Block.block + "_" + level_Block.level + "_" + level_Block.descipline + "_";
            }
            if (option_normal == false) text1 = text1 + "WEB_";
